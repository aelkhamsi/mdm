...
import { getAllGroupApplications } from "@/api/GroupApplicationApi";
...
export const DataLoader = () => {
  ...
  const setGroupApplicationsData = useSetRecoilState<any>(
  ...
  const getAdmin = () => {
    return getAdminData().then((res: any) => {
  ...
  const getUsers = () => {
    return getAllUsers().then((res: any) => {
      if (res?.statusCode === 200) {
  ...
  const getApplications = () => {
    return getAllApplications().then((res: any) => {
      if (res?.statusCode === 200) {
  ...
  const getGroupApplications = () => {
    return getAllGroupApplications().then((res: any) => {
      if (res?.statusCode === 200) {
        setGroupApplicationsData(res.applications);
      }
    });
  };
